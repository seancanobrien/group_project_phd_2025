...
\begin{remark}
    ...
    \end{itemize}
    The feature that the above have in common is that the Bass-Serre tree $T$ associated to each of them is either a point or a line.
\end{remark}
...
\begin{lemma} (\cite[2.5]{For03}, \cite[2.1]{Le07} combined)
    Let $X$ be a $G$-tree. If $\gamma \in G$ is hyperbolic then its commensurator stabilises its axis. If $X$ is a $GBS$ tree 
\end{lemma}
