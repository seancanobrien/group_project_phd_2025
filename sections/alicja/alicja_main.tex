...
\subsubsection{Isomorphism problem}
...
For the discussion in this subsection we will follow \cite[chapter I]{serre_trees_1980}, with a slight change of notation.
\begin{definition}
\label{def:graph}
    A \emph{graph} $\Gamma$ consists of 
    \begin{itemize}
        \item a set $X = \:V(\Gamma)$,
        \item a set $Y = \:E_\pm(\Gamma)$,
...
Another notion that we can associate to a graph $\Gamma$ is that of orientation. That is, an \emph{orientation} of $\Gamma$ is a subset $Y_+$ of $Y$ such that $Y$ is a disjoint union of $Y_+$ and $\overline{Y_+}$. We can then define, up to isomorphism, an \emph{oriented graph} $\Gamma_+$, by giving the two sets $X$ and $Y_+$ with a map $Y_+ \to X \times X$. The set of edges $Y$ is the disjoint union we described before. Given an oriented graph $\Gamma_+$, we may denote the positively oriented edges $E(\Gamma_+) = Y_+ = E_\pm(\Gamma_+) \setminus \overline{Y_+}$.
...
\begin{remark}
    Note, that the condition of having no circuits forces trees to have only one oriented edge between two vertices, as going along an edge $y$ and then along $\overline{y}$ would give a circuit of length 2. Thus, we can speak of a set $E(T)$ without ambiguity. 
\end{remark}

\begin{definition}\label{grpgraph}
    A graph of groups $(G,\Gamma)$ consists of a graph $\Gamma$, a group $G_p$ for each $p \in V(\Gamma)$, and a group $G_y$ for each $y \in E_\pm(\Gamma)$, together with a monomorphism $G_y \to G_{t(y)}$ (denoted $a \mapsto a^y$). In addition, it is required that $G_y = G_{\overline{y}}$.
\end{definition}

If in the above definition $\Gamma$ is a tree, then we call $(G,\Gamma)$ a tree of groups.

Another notion is one of a $G$-tree. We will connect it to the concept of a tree of groups later in this subsection.
...
\begin{definition}
    For a graph of groups $(G,\Gamma)$, the group $F(G,\Gamma)$ is generated by groups $G_p$ and the elements $y \in E_\pm(\Gamma)$ subject to relations $\overline{y} = y^{-1}$ and $ya^yy^{-1} = a^{\overline{y}}$ if $y \in E_\pm(\Gamma)$, $a \in G_y$.
\end{definition}

A more precise way to formulate this definition is to consider $\Delta$ being a free product of the groups $G_p$ and the free group with basis $E(\Gamma)$. Then $F(G,Y)$ is the quotient of $\Delta$ by the normal subgroup generated by elements $y\overline{y}$ and $ya^yy^{-1}(a^{\overline{y}})^{-1}$, $y \in E_\pm(\Gamma)$, $a \in G_y$.

\begin{definition}
    \label{def:graph_of_groups_fund_group}
    Let $T$ be a maximal tree of $\Gamma$. The \emph{fundamental group} $\pi_1(G,\Gamma,T)$ of $(G,\Gamma)$ at $T$ is the quotient of $F(G,\Gamma)$ by the normal subgroup generated by the elements $y \in E(T)$.
\end{definition}
...
\begin{construction} \label{universalcover}
    ...
    \begin{itemize}
        ...
        \item its edges are $E(\Tilde{X}_+) = \{\pi/\phi(G_y) \: | \: y \in E(\Gamma_+)\}$ where $\phi:G_y \to G_{t(y)}$ is the monomorphism which comes with $\mathbb{X}$ by definition \ref{grpgraph}, and
