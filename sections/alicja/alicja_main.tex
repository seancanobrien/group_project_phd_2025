...
\begin{construction}
    ...
    \begin{itemize}
        ...
        \item its edges are $edge \: \Tilde{X}_+ = \{\pi/G_y \: | \: y \in edge \:Y_+ \}$, where
        \item for each $gG_y \in edge \: \Tilde{X}_+$, $o(gG_y)) = gG_{o(y)}$ and $t(gG_y) = gg_yG_{t(y)}$.
    \end{itemize}
\end{construction}
...
\begin{figure}[ht]
    \centering
    \includegraphics[scale = 0.35]{sections/alicja/Tree of BS(m,n).jpeg}
...
\begin{importantexample}
    ...
    \begin{enumerate}
        ...
        \item One of the concerns that the reader could have, would be, what if $m < n$, so we have an edge labelled by $b^mG_y$ when $G_y = m\Z$. It is however important to note, that $m\Z = \langle b^m \rangle$ via the homomorphism that identifies it with its copy inside of $\Z$, so at the level of edges we do not have $b^mG_y = G_y$. \textcolor{red}{this doesn't seem right... check the construction later}
    \end{enumerate}
\end{importantexample}
...
\subsection{Generalised Baumslag-Solitar groups}
