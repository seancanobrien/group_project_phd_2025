...
\end{theorem}
\begin{proof}
    (sketch) Let $S=\{s_1,...,s_n\}$ be a generating set for $G$, and let $S^{-1}=\{s_1^{-1},...,s_n^{-1}\}$ for each $f\in \text{Aut}(G)$ define \[d(f)=\max_{S\cup S^{-1}}\lvert f(s_i)\rvert\] where $\lvert \ \rvert$
\end{proof}
