...
This theorem allows us to classify finitely presented groups which act `nicely' on $\mathbb{R}$-trees. Splittings of groups are defined in Section 4.
...
\begin{proof} (Sketch)
    Since Out$(G)$ is infinite, we can find an infinite sequence of pairwise non-conjugate automorphisms $f_i$. Define an action $\rho_i$ of $G$ on its Cayley graph by $\rho_i(g)=f_i(g)$. %\todo[color=green!40]{Perhaps you could add ``As the \(f_i\) are automorphisms,\dots,'' or something which justifies why it is easily seen.}
