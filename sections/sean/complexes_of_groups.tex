...
\todo[color=olive]{Remember to add reference}
\begin{definition}
	Suppose we have $\calx$, $G$, $G \backslash \calx$ and $p \colon G \to G \backslash \calx$ as described after \cref{def:action_of_groups_on_scwols}.\todo[color=olive]{Is it clear that this section is a well defined function? - I guess so, if you are mentioning making a choice in the paragraph below}
...
Then, because $g \in \stab(i(\tilde{a}))$, \todo[color=olive]{I might be being dumb, but it is not clear to me why $g$ is in that stabiliser} and because  $g$ acts non-degenerately,  $g \in \stab(t(\tilde{a}))$.
...
\end{definition}
\todo[color=olive]{I have noticed, that in this definition and some of the prevoius ones it's not emphasised which concept us actually being defined. Maybe it would be worth to do that?}
...
\todo[color=olive]{I am not sure why $\theta_b$ goes in brackets in the step between the lines of the equation}
\begin{align*}
	\psi_\nu\phi_a\phi_b
...
\todo[color=olive]{Is that the one that you discuss in the theorem below (3.14)? Or is this the remark that proves this theorem in one direction?}
...
\begin{proof}[Sketch of proof]
	...
	\end{align*}
    \todo[color=olive]{Missing bracket somewhere in the above line (at the end of the align)?}
	where $g,h \in G$ and $i(h\psi_{i(b)}(G_{i(b)}), b) = t(g\psi_{i(a)}(G_{i(a)}), a)$.
	There are some checks which we will omit, and refer the reader to \cite[]{BrHa11} for these in full. \todo[color=olive]{Remember to add content to [ ] in the cite}
	In particular, we should check that $t$ is a well-defined function, and that $\caly_\psi$ is well-defined as a scwol.
    \todo[color=olive]{By $t$ being a well-defined function, do you mean assigning the terminal vertex?}
	...
	Since this action acts transitively on the cosets, we get a natural isomorphism $\lambda$ between $G \backslash \caly_\psi$ and  $\calx$.\todo[color=olive]{Is "this projection" = $\lambda$(or its inverse) composed with a quotient map? Maybe it would be good to say that}
