...
Since all the $a_i$ are in $E(\catx)$, none are identity morphisms.
...
Given a scwol $\catx$, the dimension of  $\catx $ is the dimension of $\Abs{\catx}$, this is the largest  $k$ such that  $E^{(k)}(\catx) \neq \emptyset$.
...
\begin{definition}
	Morphisms of scwols are exactly functors $F \colon \catx \to \caty$ where the source and target categories are scwols..
	We say that a morphism of scwols $F \colon \catx \to \caty$ is a non-degenerate if $F$ maps $E(\catx)$ to  $E(\caty)$, and for all $v \in V(\catx)$, the restriction of  $F$ to $\Set{a \in E(\catx) \given i(a) = v}$ is a bijection on that set of edges.
\end{definition}


The requirement that $E(\catx)$ maps to $E(\caty)$ means that non-identity morphisms must be mapped to non-identity morphisms.
...
The importance of the condition that the restriction of  $F$ to $\Set{a \in E(\catx) \given i(a) = v}$ is a bijection on that set of edges will become apparent when we begin to define complexes of groups.

(Non-degenerate) automorphisms of a scwols are (non-degenerate) invertible morphisms $F \colon \catx \to \catx$.

\begin{definition}
	An action of a group $G$ on a scwol $\catx$ is a homomorphism from $G$ to the group of non-degenerate automorphisms of $\catx$ such that the following two requirements are met.
	\begin{enumerate}
		\item For all $a \in E(\catx)$ and $g \in G$, we have  $g \cdot i(a) \neq t(a)$.
		\item For all $a \in E(\catx)$ and $g \in G$, if  $g\cdot i(a)=i(a)$, then  $g\cdot a = a$.
	\end{enumerate}
	\label{def:action_of_groups_on_scwols}
\end{definition}

If $\catx$ is finite dimensional, then the first condition is guaranteed.
...
\begin{center}
	\begin{tikzpicture}
		...
		\node[FSC, label=right:{$\sigma$}] (bot) at (0,0)		{};
		\node[FSC, label=right:{$\tau$}] (mid) at ($(bot)+(0,\size)$)	{};
		\node[FSC, label=right:{$\mu$}] (top) at ($(mid)+(0,\size)$)	{};

		\draw[arrow_me=stealth] (bot) to node[right] {$b$} (mid);
		\draw[arrow_me=stealth] (mid) to node[right] {$a$} (top);
...
For each $g \in G$, denote the induced action on  $\Abs{\catx}$ as $\Abs{g}$.
The second condition means that if for some $\sigma \in V(\catx)$, we have $g \cdot \sigma = \sigma$, then $\Abs{g}$ fixes pointwise any $k$-simplex corresponding to a composable tuple $(a_1,  \ldots, a_k)$ with $i(a_1)=\sigma$.
...
Given a scwol $\catx$, a group $G$, and an action as defined in \cref{def:action_of_groups_on_scwols}, we can define the quotient scwol $G \backslash \catx$ in the following way.
The vertices of $G\backslash \catx$ are $G\backslash V(\catx)$, similarly the edges of $G \backslash \catx$ are $G\backslash E(\catx)$.
Let $p \colon \catx \to G \backslash \catx$ be the quotient map.
We make $G \backslash \catx$ in to a scwol by defining $i(p(a)) \coloneq p(i(a))$, $t(p(a)) \coloneq p(t(a))$, and where defined, $p(b)p(a)\coloneq p(ba)$.
